...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    for(auto iterator = neighbors->begin(); iterator != neighbors->end(); ++iterator){
        ...
        (*iterator)->parent  = current_node;
        (*iterator)->g_value = current_node->g_value + current_node->distance((*(*iterator)));
        (*iterator)->h_value = CalculateHValue(*iterator);
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    while(path_node != nullptr){
        ...
        path_found.push_back(*path_node);
         if( path_node->x == start_node->x && path_node->y == start_node->y){
            break;
        }
